...
public class GestionServiceImpl implements GestionService {
  ...
  public ApiPageResponse<GestionConsultaSolDetResponse> consultaDetalleSolicitud(GestionConsultaSolRequest request,
    ...
    Page<SolicitudBean> listaSolicitudes = solicitudDao.consultar(buildSolicitudBean(request),
            PageRequest.of(page - 1, size),
            request.getFechaIni(),
            request.getFechaFin());

    List<GestionConsultaSolDetResponse> listaTotal = new ArrayList<GestionConsultaSolDetResponse>();

    for (SolicitudBean solicitud : listaSolicitudes) {
